...
export const res = await fetch(`/api/ratings?companyId=${activeCompanyId}&limit=100`);

export const data = await res.json();

export const res = await fetch(`/api/ratings/dashboard?companyId=${activeCompanyId}`);

export const data = await res.json();

export const filteredRatings = ratings.filter((r) => {
    if (search) {
...
export const maxCount = Math.max(...dashboard.distribution.map((x) => x.count));

export const catCfg = categoryConfig[cat.category]

export const maxAvg = Math.max(...dashboard.topCategories.map((c) => c.avg));

export const catCfg = categoryConfig[cr.category]

export const tCfg = targetTypeConfig[r.targetType]

export const tCfg = targetTypeConfig[r.targetType]

export const catCfg = categoryConfig[r.category]

export const sCfg = surveyStatusConfig[s.status]

export const maxAvg = 5;
