...
export function DialogBlock1({ allTags, editing, editorOpen, handleSave, k, mockAuthors, mockContentTypes, runSeoAnalysis, seoPreview, setEditorOpen, toolbarActions }: { allTags: any, editing: any, editorOpen: any, handleSave: any, k: any, mockAuthors: any, mockContentTypes: any, runSeoAnalysis: any, seoPreview: any, setEditorOpen: any, toolbarActions: any }) {
  return (
    <Dialog open={editorOpen} onOpenChange={setEditorOpen}><DialogContent className="max-w-5xl max-h-[95vh] overflow-y-auto"><DialogHeader><DialogTitle>{editing ? `Izmeni: ${editing.title}` : 'Novi sadržaj'}</DialogTitle><DialogDescription>WYSIWYG editor sa SEO podrškom</DialogDescription></DialogHeader><div className="space-y-4">
            <div className="space-y-2"><Label>Naslov</Label><Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value, slug: generateSlug(e.target.value) }))} /></div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
...
export function DialogBlock0({ allTags, dialogOpen, handleSave, k, mockAuthors, mockCategories, mockContentTypes, setDialogOpen }: { allTags: any, dialogOpen: any, handleSave: any, k: any, mockAuthors: any, mockCategories: any, mockContentTypes: any, setDialogOpen: any }) {
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Novi sadržaj</DialogTitle><DialogDescription>Popunite podatke za novi sadržaj</DialogDescription></DialogHeader><div className="space-y-4">
            <div className="space-y-2"><Label>Naslov</Label><Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value, slug: generateSlug(e.target.value) }))} /></div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
...
function ContentTab({ content, setContent }: { content: ContentItem[]; setContent: React.Dispatch<React.SetStateAction<ContentItem[]>> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Create Dialog */}
              <DialogBlock0 allTags={allTags} dialogOpen={dialogOpen} handleSave={handleSave} k={k} mockAuthors={mockAuthors} mockCategories={mockCategories} mockContentTypes={mockContentTypes} setDialogOpen={setDialogOpen} />

      {/* Editor Dialog */}
              <DialogBlock1 allTags={allTags} editing={editing} editorOpen={editorOpen} handleSave={handleSave} k={k} mockAuthors={mockAuthors} mockContentTypes={mockContentTypes} runSeoAnalysis={runSeoAnalysis} seoPreview={seoPreview} setEditorOpen={setEditorOpen} toolbarActions={toolbarActions} />
    </div>
  )
}
