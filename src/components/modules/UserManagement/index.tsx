...
export function UserManagement() {
  ...
  // Tab state
  const [subTab, setSubTab] = useState<'pregled' | 'dodaj' | 'uredi'>('pregled')

  // Dialog state (AlertDialog only)
  ...
  const handleTabChange = (value: string) => {
    ...
    setSubTab(value as 'pregled' | 'dodaj' | 'uredi')
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div {...fadeInUp} className="flex items-start gap-4">
        ...
        {subTab === 'pregled' && (
          <Button onClick={() => setSubTab('dodaj')} className="gap-2 shrink-0">
            <Plus className="h-4 w-4" />
      ...
      <Tabs value={subTab} onValueChange={handleTabChange}>
        <TabsList>
          ...
          <TabsTrigger value="dodaj">Dodaj korisnika</TabsTrigger>
          {editingUser && <TabsTrigger value="uredi">Uredi ulogu</TabsTrigger>}
        </TabsList>
        ...
        <TabsContent value="pregled" className="space-y-6">
          ...
          {loading ? (
            ...
            <EmptyState onAdd={() => setSubTab('dodaj')} />
          ) : (
            <motion.div
        ...
        <TabsContent value="uredi">
          <Card className="sm:max-w-md">
            <CardHeader>
              ...
              <CardDescription>
                {editingUser && (
                  <>Izaberite novu ulogu za korisnika <span className="font-medium text-foreground">{editingUser.firstName} {editingUser.lastName}</span></>
                )}
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="flex justify-end gap-2 mt-4"><Button variant="outline" onClick={() => { setSubTab('pregled'); setEditingUser(null); setEditRoleId(''); }} disabled={editSubmitting}>Otkaži</Button><Button onClick={handleEditRole} disabled={editSubmitting || editRoleId === editingUser?.roleId} className="gap-2">{editSubmitting ? (<Loader2 className="h-4 w-4 animate-spin" />) : (<Pencil className="h-4 w-4" />)}Sačuvaj izmene</Button></div>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
