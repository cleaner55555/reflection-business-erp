...
export function Utilities() {
  ...
  const openCreate = () => { setEditItem(null); setForm({ name: '', provider: '', accountNo: '', type: 'electricity', status: 'active', monthlyAmount: 0, location: '', notes: '' }); setActiveTab('dodaj') }
  const openEdit = (item: Utility) => { setEditItem(item); setForm({ ...item }); setActiveTab('dodaj') }
  ...
  return (<div className="space-y-6"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div><h1 className="text-2xl font-bold tracking-tight">Komunalije</h1><p className="text-sm text-muted-foreground">Upravljanje komunalnim računima</p></div>{activeTab === 'pregled' && <Button size="sm" className="gap-2" onClick={openCreate}><Plus className="h-4 w-4" />Novi račun</Button>}</div>
    ...
    <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); if (v !== 'dodaj') setEditItem(null) }}><TabsList><TabsTrigger value="pregled">Pregled</TabsTrigger><TabsTrigger value="dodaj">{editItem ? 'Uredi' : 'Dodaj'}</TabsTrigger><TabsTrigger value="uredi">Uredi</TabsTrigger></TabsList>
