...
export const priorityConfig: Record<string, { labelKey: string; color: string }> = {
...
export const equipmentStatusConfig: Record<string, { labelKey: string; color: string }> = {
...
export const res = await fetch(`/api/maintenance/orders/dashboard?companyId=${activeCompanyId}`);

export const params = new URLSearchParams({ companyId: activeCompanyId });

export const res = await fetch(`/api/maintenance/orders?${params}`);
...
export const good = equipment.filter((e) => e.healthScore >= 75).length;

export const warning = equipment.filter((e) => e.healthScore >= 50 && e.healthScore < 75).length;

export const critical = equipment.filter((e) => e.healthScore >= 25 && e.healthScore < 50).length;

export const down = equipment.filter((e) => e.healthScore < 25).length;

export const now = new Date();

export const week = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);

export const today = new Date().toISOString().slice(0, 10);

export const total = monthlyTrendData.reduce((s, m) => s + m.preventive + m.corrective + m.emergency, 0);

export const prev = monthlyTrendData.reduce((s, m) => s + m.preventive, 0);
...
export const getEquipStatusBadge = (status: string) => {
...
export const getHealthBg = (score: number) => {
...
export const getNextStatusLabel = (current: string) => {
    switch (current) {
...
export const getNextStatusValue = (current: string) => {
    switch (current) {
...
export const isOverdue = plan.nextDue < new Date().toISOString().slice(0, 10);

export const isLow = part.qtyInStock <= part.minStock;
