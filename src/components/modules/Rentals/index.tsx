...
import { Plus, Search, Trash2, Pencil, Eye, Key, Calendar, AlertTriangle, ArrowLeft } from 'lucide-react'
...
export function Rentals() {
  ...
  const openCreate = () => { setEditItem(null); setForm({ contractNo: `IZN-2024-${String(data.length + 1).padStart(3, '0')}`, tenantName: '', phone: '', email: '', propertyTitle: '', propertyAddress: '', monthlyRent: 0, deposit: 0, startDate: new Date().toISOString().split('T')[0], endDate: '', paymentDay: 1, status: 'pending', lastPayment: '', nextPayment: '', paymentMethod: 'bank_transfer', securityDeposit: 0, notes: '' }); setActiveTab('dodaj') }
  const openEdit = (item: Rental) => { setEditItem(item); setForm({ ...item }); setActiveTab('uredi') }
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}><TabsList><TabsTrigger value="pregled">Pregled</TabsTrigger><TabsTrigger value="dodaj">Dodaj</TabsTrigger><TabsTrigger value="uredi">Uredi</TabsTrigger></TabsList>
        ...
        <TabsContent value="uredi" className="mt-4">{editItem ? (<Card><CardHeader><div className="flex items-center gap-2"><Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setEditItem(null)}><ArrowLeft className="h-4 w-4" /></Button><CardTitle className="text-base">Uredi ugovor — {editItem.contractNo}</CardTitle></div></CardHeader><CardContent><div className="grid gap-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div className="grid gap-2"><Label className="text-xs">Zakupec *</Label><Input className="text-xs" value={form.tenantName || ''} onChange={e => setForm({ ...form, tenantName: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Nekretnina *</Label><Input className="text-xs" value={form.propertyTitle || ''} onChange={e => setForm({ ...form, propertyTitle: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Adresa</Label><Input className="text-xs" value={form.propertyAddress || ''} onChange={e => setForm({ ...form, propertyAddress: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Status</Label><Select value={form.status || 'pending'} onValueChange={v => setForm({ ...form, status: v as Rental['status'] })}><SelectTrigger className="text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="active">Aktivan</SelectItem><SelectItem value="expiring">Ističe</SelectItem><SelectItem value="expired">Istekao</SelectItem><SelectItem value="terminated">Prekinut</SelectItem><SelectItem value="pending">Na čekanju</SelectItem></SelectContent></Select></div><div className="grid gap-2"><Label className="text-xs">Mesečna kirija (€)</Label><Input className="text-xs" type="number" value={form.monthlyRent || ''} onChange={e => setForm({ ...form, monthlyRent: Number(e.target.value) })} /></div><div className="grid gap-2"><Label className="text-xs">Depozit (€)</Label><Input className="text-xs" type="number" value={form.deposit || ''} onChange={e => setForm({ ...form, deposit: Number(e.target.value) })} /></div><div className="grid gap-2"><Label className="text-xs">Početak</Label><Input className="text-xs" type="date" value={form.startDate || ''} onChange={e => setForm({ ...form, startDate: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Kraj</Label><Input className="text-xs" type="date" value={form.endDate || ''} onChange={e => setForm({ ...form, endDate: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Dan plaćanja</Label><Input className="text-xs" type="number" value={form.paymentDay || ''} onChange={e => setForm({ ...form, paymentDay: Number(e.target.value) })} /></div><div className="grid gap-2"><Label className="text-xs">Telefon</Label><Input className="text-xs" value={form.phone || ''} onChange={e => setForm({ ...form, phone: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Email</Label><Input className="text-xs" value={form.email || ''} onChange={e => setForm({ ...form, email: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Način plaćanja</Label><Select value={form.paymentMethod || 'bank_transfer'} onValueChange={v => setForm({ ...form, paymentMethod: v as Rental['paymentMethod'] })}><SelectTrigger className="text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="bank_transfer">Bankovni prenos</SelectItem><SelectItem value="standing_order">Stalni nalog</SelectItem><SelectItem value="cash">Gotovina</SelectItem></SelectContent></Select></div></div><div className="grid gap-2"><Label className="text-xs">Napomene</Label><Input className="text-xs" value={form.notes || ''} onChange={e => setForm({ ...form, notes: e.target.value })} /></div><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => setEditItem(null)}>Otkaži</Button><Button size="sm" onClick={handleSave}>Sačuvaj</Button></div></div></CardContent></Card>) : (<Card><CardHeader><CardTitle className="text-base">Uredi ugovore</CardTitle></CardHeader><CardContent><div className="max-h-[500px] overflow-y-auto space-y-3">{data.map(item => (<div key={item.id} className="flex items-center gap-3 p-3 border rounded-lg"><div className="flex-1 min-w-0"><div className="flex items-center gap-2"><span className="text-xs font-mono">{item.contractNo}</span><span className="text-xs font-medium">{item.tenantName}</span>{getStatusBadge(item.status)}</div><p className="text-xs text-muted-foreground truncate">{item.propertyTitle} — {formatEUR(item.monthlyRent)}/mes</p></div><Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={() => openEdit(item)}><Pencil className="h-3.5 w-3.5" /></Button><Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 shrink-0" onClick={() => handleDelete(item.id)}><Trash2 className="h-3.5 w-3.5" /></Button></div>))}</div></CardContent></Card>)}</TabsContent>
      </Tabs>
      {!!detailId && detailItem && (
