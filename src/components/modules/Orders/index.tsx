...
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'; import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'; import { Label } from '@/components/ui/label'
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table'
import { Plus, Search, Trash2, Pencil, Eye, ShoppingCart, Truck, Clock, AlertTriangle, ArrowLeft } from 'lucide-react'; import { toast } from 'sonner'; import { formatDate } from '@/lib/helpers'
...
function getPriorityBadge(s: string) { const r = PRIORITIES[s]; return r ? <Badge className={`${r.color} text-xs`}>{r.label}</Badge> : <Badge className="text-xs">{s}</Badge> }
...
export function Orders() {
  ...
  const openEdit = (item: Order) => { setEditItem(item); setForm({ ...item }); setActiveTab('dodaj') }
  ...
  return (<div className="space-y-6"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div><h1 className="text-2xl font-bold tracking-tight">Narudžbe</h1><p className="text-sm text-muted-foreground">Nabavke, isporuke i interni transferi</p></div><Button size="sm" className="gap-2" onClick={openCreate}><Plus className="h-4 w-4" />Nova narudžba</Button></div>
    ...
    <Tabs value={activeTab} onValueChange={(tab) => { setActiveTab(tab); if (tab !== 'dodaj') setEditItem(null); if (tab !== 'pregled') setDetailId(null) }}><TabsList><TabsTrigger value="pregled">Pregled</TabsTrigger><TabsTrigger value="dodaj">{editItem ? 'Uredi' : 'Dodaj'}</TabsTrigger><TabsTrigger value="uredi">Uredi</TabsTrigger></TabsList>
      ...
      <TabsContent value="dodaj" className="mt-4"><Card><CardHeader><CardTitle className="text-base">{editItem ? 'Uredi narudžbu' : 'Nova narudžba'}</CardTitle></CardHeader><CardContent><div className="grid gap-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div className="grid gap-2"><Label className="text-xs">Klijent *</Label><Input className="text-xs" value={form.client || ''} onChange={e => setForm({ ...form, client: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Tip</Label><Select value={form.type || 'sale'} onValueChange={v => setForm({ ...form, type: v as Order['type'] })}><SelectTrigger className="text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="sale">Prodaja</SelectItem><SelectItem value="purchase">Nabavka</SelectItem><SelectItem value="internal">Interni</SelectItem></SelectContent></Select></div><div className="grid gap-2"><Label className="text-xs">Status</Label><Select value={form.status || 'pending'} onValueChange={v => setForm({ ...form, status: v as Order['status'] })}><SelectTrigger className="text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="pending">Na čekanju</SelectItem><SelectItem value="confirmed">Potvrđen</SelectItem><SelectItem value="processing">U obradi</SelectItem><SelectItem value="shipped">Poslat</SelectItem><SelectItem value="delivered">Isporučen</SelectItem><SelectItem value="cancelled">Otkazan</SelectItem></SelectContent></Select></div><div className="grid gap-2"><Label className="text-xs">Datum isporuke</Label><Input className="text-xs" type="date" value={form.deliveryDate || ''} onChange={e => setForm({ ...form, deliveryDate: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Magacin</Label><Input className="text-xs" value={form.warehouse || ''} onChange={e => setForm({ ...form, warehouse: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Dobavljač</Label><Input className="text-xs" value={form.supplier || ''} onChange={e => setForm({ ...form, supplier: e.target.value })} /></div><div className="grid gap-2"><Label className="text-xs">Iznos (RSD)</Label><Input className="text-xs" type="number" value={form.totalAmount || ''} onChange={e => setForm({ ...form, totalAmount: Number(e.target.value) })} /></div><div className="grid gap-2"><Label className="text-xs">Prioritet</Label><Select value={form.priority || 'medium'} onValueChange={v => setForm({ ...form, priority: v as Order['priority'] })}><SelectTrigger className="text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="low">Nizak</SelectItem><SelectItem value="medium">Srednji</SelectItem><SelectItem value="high">Visok</SelectItem><SelectItem value="urgent">Hitno</SelectItem></SelectContent></Select></div></div><div className="flex gap-2"><Button size="sm" className="gap-2" onClick={handleSave}><Plus className="h-4 w-4" />{editItem ? 'Sačuvaj' : 'Kreiraj'}</Button>{editItem && <Button variant="outline" size="sm" onClick={() => { setEditItem(null); setActiveTab('pregled') }}>Otkaži</Button>}</div></div></CardContent></Card></TabsContent>
      <TabsContent value="uredi" className="mt-4"><Card><CardHeader><CardTitle className="text-base">Uredi</CardTitle></CardHeader><CardContent><div className="max-h-[500px] overflow-y-auto space-y-3">{data.map(item => (<div key={item.id} className="flex items-center gap-3 p-3 border rounded-lg"><div className="flex-1 min-w-0"><div className="flex items-center gap-2"><span className="text-xs font-mono">{item.orderNo}</span><span className="text-xs font-medium">{item.client}</span>{getStatusBadge(item.status)}{getPriorityBadge(item.priority)}</div><p className="text-xs text-muted-foreground truncate">{formatDate(item.deliveryDate)} — {item.items} stavki</p></div><Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={() => openEdit(item)}><Pencil className="h-3.5 w-3.5" /></Button><Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 shrink-0" onClick={() => handleDelete(item.id)}><Trash2 className="h-3.5 w-3.5" /></Button></div>))}</div></CardContent></Card></TabsContent>
    </Tabs>
    { detailId && (
      <Card className="sm:max-w-[500px]">
        <CardHeader><div className="flex items-center gap-2"><Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setDetailId(null)}><ArrowLeft className="h-4 w-4" /></Button><CardTitle className="text-base">Narudžba {detailItem?.orderNo}</CardTitle></div></CardHeader>{detailItem && (<div className="space-y-3"><div className="flex items-center gap-2">{getStatusBadge(detailItem.status)}{getPriorityBadge(detailItem.priority)}</div><div className="grid grid-cols-2 gap-3">{[['Klijent', detailItem.client],['Tip', detailItem.type === 'purchase' ? 'Nabavka' : detailItem.type === 'sale' ? 'Prodaja' : 'Interni'],['Datum', formatDate(detailItem.date)],['Datum isporuke', formatDate(detailItem.deliveryDate)],['Stavki', String(detailItem.items)],['Iznos', detailItem.totalAmount > 0 ? formatRSD(detailItem.totalAmount) : '—'],['Magacin', detailItem.warehouse || '—'],['Dobavljač', detailItem.supplier || '—']].map(([l, v]) => (<div key={l} className="p-2 rounded-lg bg-muted/50"><div className="text-xs text-muted-foreground">{l}</div><div className="text-xs font-medium">{v}</div></div>))}</div></div>)}
      </Card>
    ) }
  </div>)
}
