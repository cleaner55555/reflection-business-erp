...
function getStoredLayout(): string[] {
  ...
  try {
    ...
    if (stored) {
      ...
      if (Array.isArray(parsed) && DEFAULT_DASHBOARD_LAYOUT.every(s => parsed.includes(s))) {
        return parsed
      }
    }
  } catch {}
...
// ============ SORTABLE SECTION ============
function SortableSection({ id, index, children }: { id: string; index: number; children: ReactNode }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`relative group/section rounded-xl transition-all duration-200 ${
        isDragging
          ? 'opacity-60 shadow-2xl ring-2 ring-primary/20 scale-[1.02]'
          : 'hover:shadow-sm'
      }`}
    >
      ...
      {/* Section content */}
      <div className="animate-fade-in-up" style={{ animationDelay: `${index * 40}ms`, animationFillMode: 'backwards' }}>
        {children}
      </div>
    </div>
  )
...
export function Dashboard() {
  ...
  const [sectionOrder, setSectionOrder] = useState<string[]>(() => getStoredLayout())
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(LAYOUT_STORAGE_KEY, JSON.stringify(sectionOrder))
    }
  }, [sectionOrder])
  ...
  const handleResetLayout = useCallback(() => {
    setSectionOrder([...DEFAULT_DASHBOARD_LAYOUT])
  }, [])

  // Check if layout is custom
  const isCustomLayout = sectionOrder.join(',') !== DEFAULT_DASHBOARD_LAYOUT.join(',')
        ...
        <div className="flex items-center gap-2">
          ...
          {isCustomLayout && (
            <Button
              variant="ghost"
              size="sm"
              className="h-9 gap-2 text-xs rounded-lg text-muted-foreground hover:text-foreground"
        ...
        >
          <div className="space-y-6">
            {sectionOrder.map((sectionId, index) => (
              <SortableSection key={sectionId} id={sectionId} index={index}>
