...
export function calcItemBase(item: { quantity: number; unitPrice: number; discountPct?: number }) {
  const subtotal = item.quantity * item.unitPrice
  const discount = subtotal * ((item.discountPct || 0) / 100)
  return subtotal - discount
}

export function calcItemTax(item: { quantity: number; unitPrice: number; discountPct?: number; taxRate?: number }) {
  return calcItemBase(item) * ((item.taxRate || 20) / 100)
}

export function calcItemTotal(item: { quantity: number; unitPrice: number; discountPct?: number; taxRate?: number }) {
...
export function numberToSerbian(amount: number): string {
  ...
  const tensArr = ['', '', 'dvadeset', 'trideset', 'četrdeset', 'pedeset', 'šezdeset', 'sedamdeset', 'osamdeset', 'devedeset']
  ...
  function convertChunk(n: number): string {
    ...
    else if (remainder < 20) result += (result ? ' ' : '') + teens[remainder - 10]
    else { result += (result ? ' ' : '') + tensArr[t]; if (u > 0) result += (result ? ' ' : '') + units[u] }
    return result
  }
  ...
  if (millions > 0) {
    result += millions === 1 ? 'jedan milion' : convertChunk(millions) + ' ' + getMillionWord(millions)
  }
  const thousands = Math.floor((intPart % 1000000) / 1000)
  if (thousands > 0) {
    if (result) result += ' '
    result += thousands === 1 ? 'jedna hiljada' : thousands < 5 ? convertChunk(thousands) + ' hiljade' : convertChunk(thousands) + ' hiljada'
  }
...
export function getDinarWord(n: number): string {
  const lastTwo = n % 100; const lastOne = n % 10
