...
export const emptyProductForm = { name: '', sku: '', category: 'Elektronika', lifecycleStage: 'concept', status: 'development', version: '0.1.0', owner: '', description: '' }

export const emptyRevisionForm = { productId: '', version: '', description: '', changeType: 'Minor' as string, affectedComponents: '' }

export const emptyDocForm = { title: '', productId: '', docType: 'drawing', version: '1.0', status: 'draft' }

export const emptyEcrForm = { productId: '', description: '', priority: 'medium' as string, justification: '', affectedAreas: '' }

export const matchSearch = !productSearch || p.name.toLowerCase().includes(productSearch.toLowerCase()) || p.sku.toLowerCase().includes(productSearch.toLowerCase());

export const matchCategory = productCategoryFilter === 'all' || p.category === productCategoryFilter;

export const matchStage = productStageFilter === 'all' || p.lifecycleStage === productStageFilter;

export const matchStatus = productStatusFilter === 'all' || p.status === productStatusFilter;

export const matchSearch = !revisionSearch || r.productName.toLowerCase().includes(revisionSearch.toLowerCase()) || r.description.toLowerCase().includes(revisionSearch.toLowerCase());

export const matchStatus = revisionStatusFilter === 'all' || r.status === revisionStatusFilter;

export const matchSearch = !docSearch || d.title.toLowerCase().includes(docSearch.toLowerCase()) || d.productName.toLowerCase().includes(docSearch.toLowerCase());

export const matchType = docTypeFilter === 'all' || d.docType === docTypeFilter;

export const matchStatus = docStatusFilter === 'all' || d.status === docStatusFilter;

export const matchStatus = ecrStatusFilter === 'all' || e.status === ecrStatusFilter;

export const activeCount = products.filter(p => p.status === 'active').length;

export const devCount = products.filter(p => p.status === 'development').length;

export const reviewCount = revisions.filter(r => r.status === 'submitted').length;

export const changesThisMonth = revisions.filter(r => {
      const d = new Date(r.date)
      const now = new Date()
      return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear()
    }).length;

export const avgTimeToMarket = 108;

export const openEcrs = ecrs.filter(e => ['draft', 'under_review', 'approved'].includes(e.status)).length;

export const newProduct: PLMProduct = {
      id: `p-${Date.now()}`,
      ...productForm,
...
export const product = products.find(p => p.id === revisionForm.productId);

export const newRevision: PLMRevision = {
      id: `r-${Date.now()}`,
...
export const product = products.find(p => p.id === docForm.productId);

export const newDoc: PLMDocument = {
      id: `d-${Date.now()}`,
      title: docForm.title,
...
export const product = products.find(p => p.id === ecrForm.productId);

export const newEcr: PLM_ECR = {
      id: `ecr-${Date.now()}`,
...
export const ecr = ecrs.find(e => e.id === ecrId);

export const ecoNumber = `ECO-2024-${String(ecos.length + 1).padStart(3, '0')}`;

export const newEco: PLM_ECO = {
      id: `eco-${Date.now()}`,
...
export const count = revisions.filter(r => r.status === key).length;
