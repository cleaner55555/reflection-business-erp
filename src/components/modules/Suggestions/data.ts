...
export const emptyForm = { title: '', description: '', category: 'kvalitet', priority: 'medium', estimatedSaving: '' }

export const res = await fetch(`/api/suggestions?companyId=${activeCompanyId}`);

export const d = await res.json();

export const currentUserId = currentUser?.id || 'emp-1';
...
export const handleVote = (id: string, voteType: 'for' | 'against') => {
...
export const filteredSuggestions = suggestions.filter(s => {
...
export const mySuggestions = suggestions.filter(s => s.authorId === currentUserId);

export const cfg = CATEGORY_CONFIG[cb.category]

export const sc = STATUS_CONFIG[s.status]

export const cc = CATEGORY_CONFIG[s.category]

export const pc = PRIORITY_CONFIG[s.priority]

export const sc = STATUS_CONFIG[s.status]

export const cc = CATEGORY_CONFIG[s.category]

export const totalVotes = s.votesFor + s.votesAgainst;

export const approvalPct = totalVotes > 0 ? Math.round((s.votesFor / totalVotes) * 100) : 0;

export const catSuggestions = suggestions.filter(s => s.category === key);

export const catTotal = catSuggestions.length;

export const catImplemented = catSuggestions.filter(s => s.status === 'implemented').length;

export const catApproved = catSuggestions.filter(s => s.status === 'approved').length;

export const catRejected = catSuggestions.filter(s => s.status === 'rejected').length;

export const catRate = catTotal > 0 ? Math.round((catImplemented / catTotal) * 100) : 0;

export const ps = suggestions.filter(s => s.priority === key && (s.status === 'implemented' || s.status === 'rejected'));

export const avgDays = ps.length > 0 ? Math.round(ps.reduce((sum, s) => {
                        const start = new Date(s.createdAt).getTime()
                        const end = new Date(s.implementedAt || s.updatedAt).getTime()
                        return sum + (end - start) / (1000 * 60 * 60 * 24)
                      }, 0) / ps.length) : 0;

export const catSavings = suggestions.filter(s => s.category === key && s.estimatedSaving && s.estimatedSaving > 0);

export const totalSaving = catSavings.reduce((sum, s) => sum + (s.estimatedSaving || 0), 0);

export const implSaving = catSavings.filter(s => s.status === 'implemented').reduce((sum, s) => sum + (s.estimatedSaving || 0), 0);

export const s = selectedSuggestion;
