...
export const res = await fetch(`/api/complaints?companyId=${activeCompanyId}&limit=100`);

export const data = await res.json();

export const res = await fetch(`/api/complaints/stats?companyId=${activeCompanyId}`);

export const data = await res.json();

export const filtered = complaints.filter((c) => {
...
export const handleStatusChange = async (complaint: Complaint, newStatus: string) => {
    try {
      const res = await fetch('/api/complaints', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
export const isOverdue = (deadline: string, status: string) => deadline && status !== 'resolved' && status !== 'rejected' && status !== 'cancelled' && new Date(deadline) < new Date();

export const cfg = CATEGORY_CONFIG[cat.category]

export const maxCount = Math.max(...stats.byCategory.map((c) => c.count));

export const cfg = RESOLUTION_CONFIG[r.resolution]

export const maxCount = Math.max(...stats.byResolution.map((x) => x.count));

export const maxVal = Math.max(...stats.byMonth.map((x) => Math.max(x.opened, x.resolved)));

export const sCfg = STATUS_CONFIG[c.status]

export const cCfg = CATEGORY_CONFIG[c.category]

export const pCfg = PRIORITY_CONFIG[c.priority]

export const overdue = isOverdue(c.deadline, c.status);

export const nextSt = c.status !== 'resolved' && c.status !== 'rejected' && c.status !== 'cancelled' ? getNextStatus(c.status) : null;

export const isCurrent = s === selected.status;

export const isPast = STATUS_FLOW.indexOf(selected.status) > i || selected.status === 'resolved' || selected.status === 'rejected';
