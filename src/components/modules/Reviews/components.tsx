            ...
            <div key={item.id} className="p-4 rounded-lg border space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="sm" className="h-6 text-[10px] gap-1" onClick={() => onViewDetail(item.id)}><Eye className="h-3 w-3" />Detalji</Button>
                  <Button variant="ghost" size="sm" className="h-6 text-[10px] gap-1" onClick={() => onOpenResponse(item.id)}><Reply className="h-3 w-3" />Odgovori</Button>
                  <Select value={item.status} onValueChange={v => onStatusChange(item.id, v as Review['status'])}><SelectTrigger className="h-6 w-28 text-[10px]"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(STATUSES).map(([k, v]) => <SelectItem key={k} value={k}>{v.label}</SelectItem>)}</SelectContent></Select>
