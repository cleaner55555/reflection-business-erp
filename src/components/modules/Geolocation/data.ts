...
export const res = await fetch('/api/geolokacija/employees');

export const res = await fetch('/api/geolokacija/geofences');

export const res = await fetch('/api/geolokacija/events');
...
export const openGeofenceDialog = (geofence?: Geofence) => {
    if (geofence) {
...
export const handleSaveGeofence = () => {
...
export const handleDeleteGeofence = () => {
...
export const handleToggleTracking = (employeeId: string) => {
...
export const count = employees.filter(e => e.department === dept && e.isTracked).length;

export const online = employees.filter(e => e.department === dept && e.isOnline && e.isTracked).length;

export const ta = a.lastLocationAt ? new Date(a.lastLocationAt).getTime() : 0;

export const tb = b.lastLocationAt ? new Date(b.lastLocationAt).getTime() : 0;

export const cfg = EVENT_TYPE_CONFIG[ev.eventType] || EVENT_TYPE_CONFIG.idle;

export const EventIcon = cfg.icon;
