...
export const active = MOCK_CALLS.filter((c) => c.status === 'answered').length;

export const total = MOCK_CALLS.length;

export const missed = MOCK_CALLS.filter((c) => c.status === 'missed').length;

export const answeredCalls = MOCK_CALLS.filter((c) => c.status === 'answered');

export const avgDur = answeredCalls.length > 0;

export const avgWait = Math.round(MOCK_CALLS.reduce((s, c) => s + 5, 0) / MOCK_CALLS.length);

export const extensions = MOCK_EXTENSIONS.length;

export const storageUsed = MOCK_RECORDINGS.reduce((s, r) => s + r.fileSize, 0);

export const storageTotal = 10240;

export const storagePercent = Math.round((storageUsed / storageTotal) * 100);

export const handleOpenCallDetail = (call: VoipCall) => {
    setSelectedCall(call)
    setCallNotes(call.notes || '')
    setDetailOpen(true)
  }

export const handleSaveCallNotes = () => {
    setDetailOpen(false)
...
export const handleOpenExtDialog = (ext?: Extension) => {
    setEditingExt(ext ?? null)
    setExtDialogOpen(true)
  }

export const handleOpenIvrDialog = (ivr?: IvrMenu) => {
    setEditingIvr(ivr ?? null)
    setIvrDialogOpen(true)
  }

export const toggleRecSelection = (id: string) => {
    setSelectedRecs((prev) => prev.includes(id) ? prev.filter((r) => r !== id) : [...prev, id])
  }

export const toggleSelectAllRecs = () => {
