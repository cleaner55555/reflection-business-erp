...
export const res = await fetch('/api/iot/sensors');

export const data = await res.json();

export const res = await fetch('/api/iot/alerts');

export const total = sensors.length;

export const online = sensors.filter(s => s.status === 'online').length;

export const offline = sensors.filter(s => s.status === 'offline').length;

export const warning = sensors.filter(s => s.status === 'warning').length;

export const maintenance = sensors.filter(s => s.status === 'maintenance').length;

export const avgBattery = total > 0 ? Math.round(sensors.reduce((sum, s) => sum + s.batteryLevel, 0) / total) : 0;

export const avgUptime = total > 0 ? (sensors.reduce((sum, s) => sum + s.uptime, 0) / total) : 0;

export const lowBattery = sensors.filter(s => s.batteryLevel > 0 && s.batteryLevel <= 20).length;

export const unackAlerts = alerts.filter(a => !a.acknowledged).length;

export const criticalAlerts = alerts.filter(a => a.severity === 'critical' && !a.acknowledged).length;

export const byType: Record<string, number> = {}

export const byLocation: Record<string, number> = {}

export const byGroup: Record<string, number> = {}

export const q = searchQuery.toLowerCase();

export const openNewSensor = () => {
...
export const openEditSensor = (sensor: IoTSensor) => {
...
export const handleSubmitSensor = async () => {
...
export const handleDeleteSensor = async () => {
...
export const openNewAutomation = () => {
...
export const handleSaveAutomation = () => {
...
export const cfg = SENSOR_TYPE_CONFIG[type]

export const max = Math.max(...Object.values(stats.byType), 1);

export const max = Math.max(...Object.values(stats.byLocation), 1);

export const count = stats.byGroup[g.id] || 0;

export const max = Math.max(...Object.values(stats.byGroup), 1);

export const count = sensors.filter(s => s.status === status).length;

export const pct = Math.round((count / Math.max(stats.total, 1)) * 100);

export const typeCfg = SENSOR_TYPE_CONFIG[sensor.type]

export const statCfg = STATUS_CONFIG[sensor.status]
...
export const readings = sensor.readings || []

export const minVal = Math.min(...readings.map(r => r.value));

export const maxVal = Math.max(...readings.map(r => r.value));

export const avgVal = readings.reduce((s, r) => s + r.value, 0) / readings.length;

export const chartMax = maxVal + (maxVal - minVal) * 0.2 || 1;

export const pct = Math.max(0, Math.min(100, (r.value / chartMax) * 100));

export const isLast = i === readings.length - 1;

export const sevCfg = SEVERITY_CONFIG[alert.severity]

export const typeCfg = SENSOR_TYPE_CONFIG[alert.sensorType]

export const sevCfg = SEVERITY_CONFIG[alert.severity]

export const sevCfg = SEVERITY_CONFIG[rule.severity]

export const groupSensors = sensors.filter(s => s.groupId === g.id);

export const onlineCount = groupSensors.filter(s => s.status === 'online').length;

export const statCfg = STATUS_CONFIG[s.status]

export const vals = selectedSensor.readings!.map(x => x.value);

export const cMax = Math.max(...vals) || 1;

export const pct = (r.value / cMax) * 100;
