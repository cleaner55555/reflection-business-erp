...
export const emptySkillForm = { name: '', category: 'programming', description: '' }

export const emptyEmpSkillForm = { employeeId: '', employeeName: '', skillId: '', level: 3, yearsExperience: 0 }

export const emptyCertForm = { name: '', employeeId: '', employeeName: '', skillId: '', issuedBy: '', issueDate: '', expiryDate: '', certificateNumber: '' }

export const emptyAssessForm = { employeeId: '', employeeName: '', skillId: '', previousLevel: 3, newLevel: 4, notes: '' }

export const res = await fetch(`/api/skills?companyId=${activeCompanyId}&limit=100`);

export const data = await res.json();

export const res = await fetch(`/api/skills/dashboard?companyId=${activeCompanyId}`);

export const data = await res.json();

export const filteredSkills = skills.filter((s) => {
    if (search && !s.name.toLowerCase().includes(search.toLowerCase())) return false
    if (categoryFilter !== 'all' && s.category !== categoryFilter) return false
    return true
  });

export const uniqueEmployees = [...new Map(employeeSkills.map((es) => [es.employeeId, { id: es.employeeId, name: es.employeeName, department: es.employeeDepartment }])).values()]

export const matrixSkills = skills.filter((s) => s.isActive).slice(0, 10);

export const matrixEmployees = matrixEmployee === 'all';

export const getEmployeeSkillLevel = (employeeId: string, skillId: string) => {
...
export const handleCreateSkill = async () => {
...
export const handleCreateEmployeeSkill = async () => {
...
export const handleCreateCertification = async () => {
...
export const maxCount = Math.max(...dashboard.skillsByCategory.map((c) => c.count));

export const pCfg = gapPriorityConfig[gap.priority]

export const empSkills = employeeSkills.filter((es) => es.employeeId === emp.id);

export const avgLevel = empSkills.length > 0;

export const level = getEmployeeSkillLevel(emp.id, s.id);

export const empSkills = employeeSkills.filter((es) => es.employeeId === emp.id);

export const avgLevel = empSkills.reduce((sum, es) => sum + es.level, 0) / empSkills.length;

export const catInfo = getCategoryInfo(skill.category);

export const sCfg = certStatusConfig[cert.status]

export const pCfg = gapPriorityConfig[gap.priority]

export const improved = a.newLevel > a.previousLevel;
