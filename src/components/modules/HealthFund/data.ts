...
export const res = await fetch(`/api/health-fund?companyId=${activeCompanyId}`);

export const d = await res.json();

export const openCreate = (type: 'contribution' | 'claim') => {
    setCreateType(type)
...
export const handleCreateContribution = async () => {
...
export const handleCreateClaim = async () => {
...
export const filteredContributions = contributions.filter(c => {
...
export const filteredClaims = claims.filter(cl => {
...
export const sc = STATUS_CONFIG[cl.status]

export const sc = STATUS_CONFIG[c.status]

export const sc = STATUS_CONFIG[cl.status]

export const util = yt.contributions > 0 ? Math.round((yt.claims / yt.contributions) * 100) : 0;

export const util = stats.monthlyTotal > 0 ? Math.min(100, Math.round((cat.amount / stats.monthlyTotal) * 100)) : 0;
