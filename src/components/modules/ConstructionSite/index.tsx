...
export function ConstructionSite() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setData(json.map((item: Record<string, unknown>) => ({
        ...item,
  ...
  const stats = useMemo(() => ({
    ...
    avgProgress: data.filter(d => d.status !== 'completed').length > 0 ? Math.round(data.filter(d => d.status !== 'completed').reduce((s, d) => s + d.progress, 0) / data.filter(d => d.status !== 'completed').length) : 0,
  }), [data])

  const handleDelete = useCallback(async (id: string) => {
    if (!confirm('Obrisati gradilište?')) return
    try {
      const res = await fetch(`/api/construction-sites/${id}`, { method: 'DELETE' })
      if (!res.ok) throw new Error()
