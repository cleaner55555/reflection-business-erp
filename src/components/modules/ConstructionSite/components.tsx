...
export function getTypeBadge(t: string) { const r = TYPES[t]; return r ? <Badge className={`${r.color} text-[10px]`}>{r.label}</Badge> : <Badge className="text-[10px]">{t}</Badge> }
...
export function KpiCards({ stats }: { stats: { total: number; active: number; completed: number; onHold: number; totalWorkers: number; totalBudget: number; totalSpent: number; avgProgress: number } }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
      ...
      <Card className="p-4"><div className="text-[10px] text-blue-600 mb-1">Aktivna</div><p className="text-xl font-bold text-blue-700">{stats.active}</p></Card>
      <Card className="p-4"><div className="text-[10px] text-emerald-600 mb-1">Završena</div><p className="text-xl font-bold text-emerald-700">{stats.completed}</p></Card>
      <Card className="p-4"><div className="text-[10px] text-red-600 mb-1">Pauzirana</div><p className="text-xl font-bold text-red-700">{stats.onHold}</p></Card>
      <Card className="p-4"><div className="text-[10px] text-muted-foreground mb-1">Radnici</div><p className="text-xl font-bold">{stats.totalWorkers}</p></Card>
      <Card className="p-4"><div className="text-[10px] text-muted-foreground mb-1">Budžet</div><p className="text-xl font-bold">{formatCurrency(stats.totalBudget)}</p></Card>
      <Card className="p-4"><div className="text-[10px] text-muted-foreground mb-1">Potrošeno</div><p className="text-xl font-bold">{formatCurrency(stats.totalSpent)}</p></Card>
      <Card className="p-4"><div className="text-[10px] text-muted-foreground mb-1">Prosek napr.</div><p className="text-xl font-bold">{stats.avgProgress}%</p></Card>
    </div>
  )
}
            ...
            <TableBody>
              {filtered.length === 0 ? <TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground text-sm">Nema gradilišta</TableCell></TableRow> : filtered.map(item => (
                <TableRow key={item.id} className="cursor-pointer hover:bg-muted/50" onClick={() => onView(item)}>
                  ...
                  <TableCell><div className="text-xs font-medium">{item.name}</div><div className="text-[10px] text-muted-foreground">{item.address}</div></TableCell>
                  <TableCell className="text-xs hidden sm:table-cell">{item.city}</TableCell>
                  <TableCell>{getTypeBadge(item.type)}</TableCell>
                  <TableCell>{getStatusBadge(item.status)}</TableCell>
                  <TableCell className="hidden md:table-cell"><div className="w-20"><div className="flex justify-between text-[10px] mb-0.5"><span>{item.progress}%</span></div><Progress value={item.progress} className="h-1.5" /></div></TableCell>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <Card><CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2"><DollarSign className="h-4 w-4 text-emerald-600" />Budžet po projektu</CardTitle></CardHeader><CardContent className="space-y-2 max-h-64 overflow-y-auto">
        {data.map(d => <div key={d.id} className="flex items-center justify-between p-2 rounded-lg bg-muted/50"><div><p className="text-xs font-medium">{d.name}</p><p className="text-[10px] text-muted-foreground">{d.city}</p></div><div className="text-right"><p className="text-xs font-bold">{formatCurrencyFull(d.spent)}</p><p className="text-[10px] text-muted-foreground">od {formatCurrencyFull(d.budget)}</p></div></div>)}
      </CardContent></Card>
      <Card><CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-amber-600" />Aktivni zadaci</CardTitle></CardHeader><CardContent className="space-y-2 max-h-64 overflow-y-auto">
        {data.flatMap(d => d.tasks.filter(t => t.status !== 'completed').map(t => ({ ...t, site: d.name }))).map((t, i) => (
          <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-muted/50"><div><p className="text-xs font-medium">{t.name}</p><p className="text-[10px] text-muted-foreground">{t.site}</p></div><div className="flex items-center gap-2"><Badge className={`${t.status === 'in_progress' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-700'} text-[10px]`}>{t.status === 'in_progress' ? 'U toku' : 'Čeka'}</Badge><span className="text-[10px] text-muted-foreground">{formatDate(t.deadline)}</span></div></div>
        ))}
      </CardContent></Card>
    </div>
  )
        ...
        {detailItem && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Investitor</div><p className="text-xs font-medium">{detailItem.investor}</p></div>
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Izvođač</div><p className="text-xs font-medium">{detailItem.contractor}</p></div>
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Rukovodilac</div><p className="text-xs font-medium">{detailItem.projectManager}</p></div>
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Lokacija</div><p className="text-xs">{detailItem.city}</p><p className="text-[10px] text-muted-foreground">{detailItem.address}</p></div>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Radnika</div><p className="text-xs font-bold">{detailItem.workers}</p></div>
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Površina</div><p className="text-xs font-bold">{detailItem.area > 0 ? `${detailItem.area} m²` : '-'}</p></div>
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Spratova</div><p className="text-xs font-bold">{detailItem.floors || '-'}</p></div>
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Jedinica</div><p className="text-xs font-bold">{detailItem.units || '-'}</p></div>
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Početak</div><p className="text-xs">{formatDate(detailItem.startDate)}</p></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-lg bg-muted/50"><div className="text-[10px] text-muted-foreground mb-1">Budžet</div><p className="text-sm font-bold">{formatCurrencyFull(detailItem.budget)}</p><p className="text-[10px] text-muted-foreground">Potrošeno: {formatCurrencyFull(detailItem.spent)}</p></div>
              <div className="p-3 rounded-lg bg-emerald-50 dark:bg-emerald-950/20"><div className="text-[10px] text-emerald-600 mb-1">Napredak</div><p className="text-sm font-bold text-emerald-700">{detailItem.progress}%</p><Progress value={detailItem.progress} className="h-2 mt-1" /></div>
            </div>
