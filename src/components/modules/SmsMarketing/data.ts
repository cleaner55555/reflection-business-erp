...
export const res = await fetch(`/api/sms/campaigns?companyId=${activeCompanyId}&limit=100`);

export const totalCampaigns = campaigns.length;

export const totalSent = campaigns.reduce((s, c) => s + c.sentCount, 0);

export const totalDelivered = campaigns.reduce((s, c) => s + c.deliveredCount, 0);

export const totalFailed = campaigns.reduce((s, c) => s + c.failedCount, 0);

export const totalReplies = campaigns.reduce((s, c) => s + c.replyCount, 0);

export const totalCost = campaigns.reduce((s, c) => s + c.totalCost, 0);

export const deliveryRate = totalSent > 0 ? Math.round((totalDelivered / totalSent) * 100) : 0;

export const activeContacts = contacts.filter(c => c.status === 'active').length;

export const unsubscribed = contacts.filter(c => c.status === 'unsubscribed').length;

export const byCategory: Record<string, number> = {}
...
export const handleCreateKeyword = () => {
...
export const handleToggleKeyword = (id: string) => {
    setKeywords(prev => prev.map(k => k.id === id ? { ...k, enabled: !k.enabled } : k))
    showToast('Ključna reč ažurirana')
  }

export const max = Math.max(...Object.values(stats.byCategory), 1);

export const count = stats.byStatus[status] || 0;

export const count = Math.floor(Math.random() * 200) + (i > 20 ? Math.floor(Math.random() * 300) : 0);

export const maxH = 500;

export const deliveryPct = c.recipientCount > 0 ? Math.round((c.deliveredCount / c.recipientCount) * 100) : 0;

export const replyPct = c.sentCount > 0 ? Math.round((c.replyCount / c.sentCount) * 100) : 0;

export const maxTpl = Math.max(...templates.map(t => t.usedCount), 1);

export const count = contacts.filter(c => c.groups.includes(group)).length;

export const maxKw = Math.max(...keywords.map(k => k.matchCount), 1);
