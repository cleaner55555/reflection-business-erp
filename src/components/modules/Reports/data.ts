...
export const paymentMethodData = [
  ...
  { name: "Račun", value: Math.round(totalRev * 0.4) },
...
export const incomeCategories = [
  ...
  { category: "Ostali prihodi", amount: Math.round(totalRev * 0.1) },
];

export const expenseCategories = [
  { category: "Nabavka robe", amount: Math.round(kpis.totalExpenses * 0.55) },
  {
    category: "Plata zaposlenih",
    amount: Math.round(kpis.totalExpenses * 0.25),
  },
...
export const cashFlow = [
  {
    activity: "Operativni",
    inflow: Math.round(totalRev * 0.9),
    outflow: Math.round(kpis.totalExpenses * 0.85),
    net: 0,
  },
  {
    activity: "Investicioni",
    inflow: 0,
    outflow: Math.round(totalRev * 0.08),
    net: 0,
  },
  {
    activity: "Finansijski",
    inflow: Math.round(totalRev * 0.05),
    outflow: Math.round(kpis.totalExpenses * 0.03),
    net: 0,
  },
];

export const taxSummary = {
  vatCollected: Math.round(totalRev * 0.2),
  vatPaid: Math.round(kpis.totalExpenses * 0.2),
};

export const salesByPartner = partners
  .slice(0, 20)
  .map((p) => ({
    name: p.name,
    invoiceCount: p._count.invoices,
    totalRevenue: Math.round(
      ((p._count.invoices * totalRev) / Math.max(partners.length, 1)) *
        (0.5 + Math.random()),
    ),
  }))
  .sort((a, b) => b.totalRevenue - a.totalRevenue);

export const salesByProduct = products
  .slice(0, 15)
  .map((p) => ({
...
export const stockValuation = Array.from(categorySalesMap.entries())
  .map(([name]) => {
    const catProducts = products.filter(
      (p) => (p.category || "Bez kategorije") === name,
    );
...
export const inventoryTurnover = products
  .slice(0, 10)
  .map((p) => ({
...
export const partnerActivity = partners.map((p) => ({
  ...
  totalAmount: Math.round(
    (((p._count.invoices + p._count.purchaseOrders) * totalRev) /
      Math.max(partners.length, 1)) *
      (0.3 + Math.random() * 1.4),
  ),
  lastActivity: new Date(Date.now() - Math.random() * 90 * 86400000)
    .toISOString()
    .slice(0, 10),
}));
...
export const partnerTypeDistribution = Array.from(partnerTypeMap.entries()).map(
...
export const employeeData = employeeNames.map((name, i) => ({
  ...
  revenueGenerated: Math.round(
    (totalRev / employeeNames.length) * (0.3 + Math.random() * 1.4),
  ),
